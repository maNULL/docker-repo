...
```    
##### Структура каталогов зеркала
```bash
mkdir -p /docker_repo/debian/{\
    debian_bullseye_main,\
    debian_bullseye_security,\
    debian_bullseye_updates,\
    debian_bullseye_backports,\
    pve_bullseye_no_subscription,\
    pbs_bullseye_no_subscription,\
    pmg_bullseye_no_subscription\
    }/.pool
